...
def harris_corners(img, window_size=3, k=0.04):
    ...
    # Compute sum of products of derivatives at each pixel
    ...
    for i in range(H):
        for j in range(W):
            # Define the matrix M at each pixel
            M = np.array([[Sxx[i, j], Sxy[i, j]],
                          [Sxy[i, j], Syy[i, j]]])
            
            # Compute the response R at each pixel
...
def simple_descriptor(patch):
    ...
    # Normalize the patch and flatten it into a vector
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    ### YOUR CODE HERE
    for i, distances_to_desc2 in enumerate(dists):
        # Sort the distances
        sorted_indices = np.argsort(distances_to_desc2)
        closest, second_closest = sorted_indices[:2]

        # Check the ratio between the smallest and the second smallest distances
        if distances_to_desc2[closest] < threshold * distances_to_desc2[second_closest]:
            matches.append([i, closest])
    # Convert the list of matches to a NumPy array
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    for i in range(rows):
        for j in range(cols):
            cell_magnitudes = G_cells[i, j]
            cell_orientations = theta_cells[i, j]
            # Calculate histogram for the gradients in this cell
            histogram, _ = np.histogram(cell_orientations,
                                        bins=n_bins,
                                        range=(0, 180),
    ...
    block = histograms.flatten()

    # Normalize flattened block
    block = block / np.linalg.norm(block, ord=2) if np.linalg.norm(block, ord=2) > 0 else block
    ### END YOUR CODE
